...
class UAVEnv_PX4(gym.Env):

    def __init__(
        self,
        index = 0,  # env index
        rate = 10,
        steps_thr = 1024*2,  # simulation refresh rate
    ):
        super().__init__()
        self.index = index
        ...
        self.act_bnd = {"vx": (-0.1, 0.1), "vy": (-0.1, 0.1), "vz":(-0.1, 0.1)} # What are the boudaries for the velocity and angular velocity

        self.obs_dim = 10  # [dist, UAVPose(3), WaypointPose(3), prev_action(4), velocity(4)]
        self.observation_space = spaces.Box(
            low=-np.ones(self.obs_dim), high=np.ones(self.obs_dim), dtype=np.float32
        )
        self.obs_bnd = {"xyz": 10.}  # cubic space between (-100 to 100)
        ...
        self.dist_threshold = 0.5  # done if dist < dist_threshold, final goal radius distance
    ...
    def _gazebo_pose_callback(self, msg:ModelStates):
        ...
        try:
            iris_index = model_name.index("iris0")
        except ValueError:
            rospy.logwarn("'iris' model not found in ModelStates")
            return
        ...
        # Print the pose information
        self.uav_pose = np.array([iris_pose.position.x, iris_pose.position.y, iris_pose.position.z])
        self.uav_velocity = np.array([iris_velocity.linear.x, iris_velocity.linear.y, iris_velocity.linear.z])
...
class UAVEnv_Sentinel(gym.Env):
    ...
    def reset(self): # reset the training  - send the vehicle to init postion ToDo
        ...
        self.set_uav_mode("OFFBOARD")
        self.takeoff_publisher.publish(Bool(True))
        print("Taking off...")
        time.sleep(5)
        self.stop_publisher.publish(Bool(False))
        time.sleep(1)
    ...
    def _create_ros_pub_sub_srv(self):
        ...
        self.agent_actions_publisher = rospy.Publisher("uav_" + str(self.index) + "/agent/actions", TwistStamped, queue_size=1)
        self.stop_publisher = rospy.Publisher("uav_" + str(self.index) + "/agent/stop", Bool, queue_size=1)
        self.takeoff_publisher = rospy.Publisher("uav_" + str(self.index) + "/agent/takeoff", Bool, queue_size=1)
    ...
    def _mavros_vel_callback(self, msg:PointStamped):
        ...
        # Print the pose information
        self.uav_velocity = np.array([iris_velocity.x, -iris_velocity.y, -iris_velocity.z])
        #rospy.loginfo("Iris Pose:\n{}".format(self.uav_pose))
